...
- (Canary)  **optional** `ref`: A ref object (e.g. from [`useRef`](useRef.md)) or [callback function](../react-dom/components/common.md#`ref` callback function). React provides a `FragmentInstance` as the ref value that implements methods for interacting with the DOM nodes wrapped by the Fragment.
...
* (Canary)  If you want to pass `ref` to a Fragment, you can't use the `<>...</>` syntax. You have to explicitly import `Fragment` from `'react'` and render `<Fragment ref={yourRef}>...</Fragment>`.

---

### <CanaryBadge /> `FragmentInstance`
...
* [`addEventListener`](#`addEventListener%28type, listener, options?%29`) and [`removeEventListener`](#`removeEventListener%28type, listener, options?%29`) manage event listeners across all first-level DOM children.
* [`dispatchEvent`](#`dispatchEvent%28event%29`) dispatches an event on the Fragment, which can bubble to the DOM parent.
* [`focus`](#`focus%28options?%29`), [`focusLast`](#`focusLast%28options?%29`), and [`blur`](#`blur%28%29`) manage focus across all nested children depth-first.
* [`observeUsing`](#`observeUsing%28observer%29`) and [`unobserveUsing`](#`unobserveUsing%28observer%29`) attach and detach `IntersectionObserver` or `ResizeObserver` instances.
* [`getClientRects`](#`getClientRects%28%29`) returns bounding rectangles of all first-level DOM children.
* [`getRootNode`](#`getRootNode%28options?%29`) returns the root node of the Fragment's parent.
* [`compareDocumentPosition`](#`compareDocumentPosition%28otherNode%29`) compares the Fragment's position with another node.
* [`scrollIntoView`](#`scrollIntoView%28alignToTop?%29`) scrolls the Fragment's children into view.
...
* `observer`: The same `IntersectionObserver` or `ResizeObserver` instance previously passed to [`observeUsing`](#`observeUsing%28observer%29`).
...
A bitmask of [position flags](https://developer.mozilla.org/en-US/docs/Web/API/Node/compareDocumentPosition#return_value). Empty Fragments and Fragments with children rendered through a [portal](../react-dom/createPortal.md) include `Node.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC` in the result.
...
* React does not apply event listeners added via `addEventListener` to hidden [`<Activity>`](Activity.md) trees. When an `Activity` boundary switches from hidden to visible, listeners are applied automatically.
* Each first-level DOM child of a Fragment with a `ref` gets a `reactFragments` property—a `Set<FragmentInstance>` containing all Fragment instances that own the element. This enables [caching a shared observer](#<CanaryBadge /> Caching a global IntersectionObserver) across multiple Fragments.
...
### <CanaryBadge /> Adding event listeners without a wrapper element

Fragment `ref`s let you add event listeners to a group of elements without adding a wrapper DOM node. Use a [ref callback](../react-dom/components/common.md#`ref` callback function) to attach and clean up listeners:
...
### <CanaryBadge /> Managing focus across a group of elements
...
### <CanaryBadge /> Scrolling a group of elements into view
...
### <CanaryBadge /> Observing visibility without a wrapper element
...
### <CanaryBadge /> Caching a global IntersectionObserver
