...
* **optional** `options`: An object with static generation options.
  ...
  * **optional** `onError`: A callback that fires whenever there is a server error, whether [recoverable](../server/renderToPipeableStream.md#Recovering from errors outside the shell) or [not.](../server/renderToPipeableStream.md#Recovering from errors inside the shell) By default, this only calls `console.error`. If you override it to [log crash reports,](../server/renderToPipeableStream.md#Logging crashes on the server) make sure that you still call `console.error`. You can also use it to [adjust the status code](../server/renderToPipeableStream.md#Setting the status code) before the shell is emitted.
...
- If rendering the is successful, the Promise will resolve to an object containing:
  ...
  - `postponed`: a JSON-serializeable, opaque object that can be passed to [`resumeToPipeableStream`](../server/resumeToPipeableStream.md) if `prerenderToNodeStream` did not finish. Otherwise `null` indicating that the `prelude` contains all the content and no resume is necessary.
- If rendering fails, the Promise will be rejected. [Use this to output a fallback shell.](../server/renderToPipeableStream.md#Recovering from errors inside the shell)
...
This will produce the initial non-interactive HTML output of your React components. On the client, you will need to call [`hydrateRoot`](../client/hydrateRoot.md) to *hydrate* that server-generated HTML and make it interactive.
...
This can be used for partial prerendering together with [`resumeToPipeableStream`](../server/resumeToPipeableStream.md) or [`resumeAndPrerenderToNodeStream`](resumeAndPrerenderToNodeStream.md).
...
To stream content as it loads, use a streaming server render API like [renderToPipeableStream](../server/renderToPipeableStream.md).
