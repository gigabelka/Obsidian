...
* The effect of the directive depends on your [`compilationMode`](../compilationMode.md) setting.

### How `"use memo"` marks functions for optimization

In a React app that uses the React Compiler, functions are analyzed at build time to determine if they can be optimized. By default, the compiler automatically infers which components to memoize, but this can depend on your [`compilationMode`](../compilationMode.md) setting if you've set it.
...
* [`"use no memo"`](use-no-memo.md) - Opt out of compilation
* [`compilationMode`](../compilationMode.md) - Configure compilation behavior
* [React Compiler](../../../learn/react-compiler/index.md) - Getting started guide
