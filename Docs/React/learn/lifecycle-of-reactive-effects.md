...
**It's a good way to think about components, but _not_ about Effects.** Instead, try to think about each Effect independently from your component's lifecycle. An Effect describes how to [synchronize an external system](synchronizing-with-effects.md) to the current props and state. As your code changes, synchronization will need to happen more or less often.
...
> [!note]
>
> Some Effects don't return a cleanup function at all. [More often than not,](synchronizing-with-effects.md#How to handle the Effect firing twice in development?) you'll want to return one--but if you don't, React will behave as if you returned an empty cleanup function.
>
...
This might remind you how you don't think whether a component is mounting or updating when you write the rendering logic that creates JSX. You describe what should be on the screen, and React [figures out the rest.](reacting-to-input-with-state.md)
...
**React verifies that your Effect can re-synchronize by forcing it to do that immediately in development.** This might remind you of opening a door and closing it an extra time to check if the door lock works. React starts and stops your Effect one extra time in development to check [you've implemented its cleanup well.](synchronizing-with-effects.md#How to handle the Effect firing twice in development?)
...
You might be wondering how React knew that your Effect needed to re-synchronize after `roomId` changes. It's because *you told React* that its code depends on `roomId` by including it in the [list of dependencies:](synchronizing-with-effects.md#Step 2: Specify the Effect dependencies)
...
In the above example, deleting one Effect wouldn’t break the other Effect's logic. This is a good indication that they synchronize different things, and so it made sense to split them up. On the other hand, if you split up a cohesive piece of logic into separate Effects, the code may look "cleaner" but will be [more difficult to maintain.](you-might-not-need-an-effect.md#Chains of computations) This is why you should think whether the processes are same or separate, not whether the code looks cleaner.
...
Let's say that the user can pick a chat server in the dropdown, but they can also configure a default server in settings. Suppose you've already put the settings state in a [context](scaling-up-with-reducer-and-context.md) so you read the `settings` from that context. Now you calculate the `serverUrl` based on the selected server from props and the default server:
...
If your linter is [configured for React,](editor-setup.md#Linting) it will check that every reactive value used by your Effect's code is declared as its dependency. For example, this is a lint error because both `roomId` and `serverUrl` are reactive:
...
> [!note]
>
> In some cases, React *knows* that a value never changes even though it's declared inside the component. For example, the [`set` function](../reference/react/useState.md#`set` functions, like `setSomething%28nextState%29`) returned from `useState` and the ref object returned by [`useRef`](../reference/react/useRef.md) are *stable*--they are guaranteed to not change on a re-render. Stable values aren't reactive, so you may omit them from the list. Including them is allowed: they won't change, so it doesn't matter.
>
...
* **Check that your Effect represents an independent synchronization process.** If your Effect doesn't synchronize anything, [it might be unnecessary.](you-might-not-need-an-effect.md) If it synchronizes several independent things, [split it up.](#Each Effect represents a separate synchronization process)

* **If you want to read the latest value of props or state without "reacting" to it and re-synchronizing the Effect,** you can split your Effect into a reactive part (which you'll keep in the Effect) and a non-reactive part (which you'll extract into something called an _Effect Event_). [Read about separating Events from Effects.](separating-events-from-effects.md)
...
> ```
>
> On the [next](separating-events-from-effects.md) [pages](removing-effect-dependencies.md), you'll learn how to fix this code without breaking the rules. It's always worth fixing!
>
...
You'll learn a more general approach to this type of problem in [Separating Events from Effects.](separating-events-from-effects.md)
