...
`tsconfig.json` files have a new top-level property, [`references`](../tsconfig/references.md). It's an array of objects that specifies projects to reference:
...
- If the referenced project produces an [`outFile`](../tsconfig/outFile.md), the output file `.d.ts` file's declarations will be visible in this project
...
Referenced projects must have the new [`composite`](../tsconfig/composite.md) setting enabled.
This setting is needed to ensure TypeScript can quickly determine where to find the outputs of the referenced project.
Enabling the [`composite`](../tsconfig/composite.md) flag changes a few things:

- The [`rootDir`](../tsconfig/rootDir.md) setting, if not explicitly set, defaults to the directory containing the `tsconfig` file
- All implementation files must be matched by an [`include`](../tsconfig/include.md) pattern or listed in the [`files`](../tsconfig/files.md) array. If this constraint is violated, `tsc` will inform you which files weren't specified
- [`declaration`](../tsconfig/declaration.md) must be turned on

## `declarationMap`

We've also added support for [declaration source maps](https://github.com/Microsoft/TypeScript/issues/14479).
If you enable [`declarationMap`](../tsconfig/declarationMap.md), you'll be able to use editor features like "Go to Definition" and Rename to transparently navigate and edit code across project boundaries in supported editors.
...
- [`--verbose`](../tsconfig/verbose.md): Prints out verbose logging to explain what's going on (may be combined with any other flag)
- `--dry`: Shows what would be done but doesn't actually build anything
- `--clean`: Deletes the outputs of the specified projects (may be combined with `--dry`)
- [`--force`](../tsconfig/force.md): Act as if all projects are out of date
- `--watch`: Watch mode (may not be combined with any flag except [`--verbose`](../tsconfig/verbose.md))

## Caveats

Normally, `tsc` will produce outputs (`.js` and `.d.ts`) in the presence of syntax or type errors, unless [`noEmitOnError`](../tsconfig/noEmitOnError.md) is on.
...
For this reason, `tsc -b` effectively acts as if [`noEmitOnError`](../tsconfig/noEmitOnError.md) is enabled for all projects.

If you check in any build outputs (`.js`, `.d.ts`, `.d.ts.map`, etc.), you may need to run a [`--force`](../tsconfig/force.md) build after certain source control operations depending on whether your source control tool preserves timestamps between the local copy and the remote copy.
...
With more `tsconfig.json` files, you'll usually want to use [Configuration file inheritance](tsconfig.json.md) to centralize your common compiler options.
This way you can change a setting in one file rather than having to edit multiple files.

Another good practice is to have a "solution" `tsconfig.json` file that simply has [`references`](../tsconfig/references.md) to all of your leaf-node projects and sets [`files`](../tsconfig/files.md) to an empty array (otherwise the solution file will cause double compilation of files). Note that starting with 3.0, it is no longer an error to have an empty [`files`](../tsconfig/files.md) array if you have at least one `reference` in a `tsconfig.json` file.
...
You will need to either set the [`outDir`](../tsconfig/outDir.md) to an explicit subfolder of the output folder, or set the [`rootDir`](../tsconfig/rootDir.md) to the common root of all project folders.

### Structuring for outFiles

Layout for compilations using [`outFile`](../tsconfig/outFile.md) is more flexible because relative paths don't matter as much.
