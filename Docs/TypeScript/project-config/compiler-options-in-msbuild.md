...
The new `tsconfig.json` will then be used as the source of truth for TypeScript-specific build information like files and configuration. You can learn [about how TSConfigs works here](tsconfig.json.md) and there is a [comprehensive reference here](../tsconfig/intro.md).
...
There is a series of mappings for common TypeScript settings, these are settings which map directly to [TypeScript cli options](compiler-options.md) and are used to help you write a more understandable project file. You can use the [TSConfig reference](../tsconfig/intro.md) to get more information on what values and defaults are for each mapping.
...
For example, this would turn on [`noPropertyAccessFromIndexSignature`](../tsconfig/noPropertyAccessFromIndexSignature.md):
...
However, if this behavior is causing issues, such as when TypeScript's [`incremental`](../tsconfig/incremental.md) option is enabled, set `<TypeScriptEnableIncrementalMSBuild>false</TypeScriptEnableIncrementalMSBuild>` to ensure the TypeScript compiler is invoked with every run of MSBuild.
