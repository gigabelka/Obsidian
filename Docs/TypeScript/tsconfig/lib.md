...
TypeScript also includes APIs for newer JS features matching the [`target`](target.md) you specify; for example the definition for `Map` is available if [`target`](target.md) is `ES6` or newer.
