...
You can use most JSDoc type syntax and any TypeScript syntax, from [the most basic like `string`](../handbook/basics.md) to [the most advanced, like conditional types](../handbook/type-manipulation/conditional-types.md).
...
`@satisfies` provides access to the postfix [operator `satisfies`](../release-notes/typescript-4.9.md) in TypeScript. Satisfies is used to declare that a value implements a type but does not affect the type of the value.
...
> Note: Error messages only show up in JS codebases with [a JSConfig](../project-config/tsconfig.json.md) and [`checkJs`](../tsconfig/checkJs.md) enabled.
...
Nullable types only have meaning if [`strictNullChecks`](../tsconfig/strictNullChecks.md) is on:
