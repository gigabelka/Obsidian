...
This feature is something most users may never have to think about; however, if you've hit issues under [`isolatedModules`](../tsconfig/isolatedModules.md), TypeScript's `transpileModule` API, or Babel, this feature might be relevant.
...
In conjunction with `import type`, TypeScript 3.8 also adds a new compiler flag to control what happens with imports that won't be utilized at runtime: [`importsNotUsedAsValues`](../tsconfig/importsNotUsedAsValues.md).
...
Currently, you can only use top level `await` when the [`target`](../tsconfig/target.md) compiler option is `es2017` or above, and `module` is `esnext` or `system`.
...
TypeScript 3.8 supports `es2020` as an option for `module` and [`target`](../tsconfig/target.md).
...
It also means `bigint` literals now have a stable [`target`](../tsconfig/target.md) below `esnext`.

## JSDoc Property Modifiers

TypeScript 3.8 supports JavaScript files by turning on the [`allowJs`](../tsconfig/allowJs.md) flag, and also supports _type-checking_ those JavaScript files via the [`checkJs`](../tsconfig/checkJs.md) option or by adding a `// @ts-check` comment to the top of your `.js` files.
...
- [`watchFile`](../tsconfig/watchFile.md): the strategy for how individual files are watched. This can be set to
...
- [`watchDirectory`](../tsconfig/watchDirectory.md): the strategy for how entire directory trees are watched under systems that lack recursive file-watching functionality. This can be set to:

  - `fixedPollingInterval`: Check every directory for changes several times a second at a fixed interval.
  - `dynamicPriorityPolling`: Use a dynamic queue where less-frequently modified directories will be checked less often.
  - `useFsEvents` (the default): Attempt to use the operating system/file system's native events for directory changes.

- [`fallbackPolling`](../tsconfig/fallbackPolling.md): when using file system events, this option specifies the polling strategy that gets used when the system runs out of native file watchers and/or doesn't support native file watchers. This can be set to
...
TypeScript 3.8 introduces a new compiler option called [`assumeChangesOnlyAffectDirectDependencies`](../tsconfig/assumeChangesOnlyAffectDirectDependencies.md).
...
Under [`assumeChangesOnlyAffectDirectDependencies`](../tsconfig/assumeChangesOnlyAffectDirectDependencies.md), a change in `fileA.ts` means that only `fileA.ts` and `fileB.ts` need to be re-checked.
