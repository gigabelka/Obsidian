...
Note that in [`strictNullChecks`](../tsconfig/strictNullChecks.md) mode, when a homomorphic mapped type removes a `?` modifier from a property in the underlying type it also removes `undefined` from the type of that property:
...
JSX factory can be configured for a compilation using [`jsxFactory`](../tsconfig/jsxFactory.md) (default is `React.createElement`). With TypeScript 2.8 you can override this on a per-file-basis by adding a comment to the beginning of the file.
...
[`emitDeclarationOnly`](../tsconfig/emitDeclarationOnly.md) allows for _only_ generating declaration files; `.js`/`.jsx` output generation will be skipped with this flag. The flag is useful when the `.js` output generation is handled by a different transpiler like Babel.
