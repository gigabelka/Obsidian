...
For this reason, TypeScript uses a simpler emit by default that only supports array types, and supports iterating on other types using the [`downlevelIteration`](../tsconfig/downlevelIteration.md) flag.
The looser default without [`downlevelIteration`](../tsconfig/downlevelIteration.md) works fairly well; however, there were some common cases where the transformation of array spreads had observable differences.
...
TypeScript 3.6 introduces a new `__spreadArrays` helper to accurately model what happens in ECMAScript 2015 in older targets outside of [`downlevelIteration`](../tsconfig/downlevelIteration.md).
...
Additionally, TypeScript 3.4 introduced the [`incremental`](../tsconfig/incremental.md) flag for saving information about previous compilations to only rebuild certain files.
...
For creating [`incremental`](../tsconfig/incremental.md) builds, users can leverage the `createIncrementalProgram` and `createIncrementalCompilerHost` APIs.
...
- The [`target`](../tsconfig/target.md) option (allowing users to switch out of `es5` to `es3`, `es2015`, `esnext`, etc.)
- All the strictness flags (including just [`strict`](../tsconfig/strict.md))
- Support for plain JavaScript files (using `allowJS` and optionally [`checkJs`](../tsconfig/checkJs.md))

These options also persist when sharing links to playground samples, allowing users to more reliably share examples without having to tell the recipient "oh, don't forget to turn on the [`noImplicitAny`](../tsconfig/noImplicitAny.md) option!".
