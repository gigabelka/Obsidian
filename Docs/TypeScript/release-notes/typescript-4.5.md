...
While there are some reasonable defaults based on your [`target`](../tsconfig/target.md), you can pick and choose which declaration files your program uses by configuring the [`lib`](../tsconfig/lib.md) setting in the `tsconfig.json`.
...
The main feature in [`module es2022`](../tsconfig/module.md) is top-level `await`, meaning you can use `await` outside of `async` functions.
This was already supported in `--module esnext` (and now [`--module nodenext`](../tsconfig/target.md)), but `es2022` is the first stable target for this feature.
...
In TypeScript 4.5, you can enable a new flag called [`preserveValueImports`](../tsconfig/preserveValueImports.md) to prevent TypeScript from stripping out any imported values from your JavaScript outputs.
...
With TypeScript 4.5, you can use [`preserveValueImports`](../tsconfig/preserveValueImports.md) to avoid these situations.

Note that this flag has a special requirement when combined with [--isolatedModules`](../tsconfig/isolatedModules.md): imported
...
As mentioned above, [`preserveValueImports`](../tsconfig/preserveValueImports.md) and [`isolatedModules`](../tsconfig/isolatedModules.md) have special requirements so that there's no ambiguity for build tools whether it's safe to drop type imports.
...
In the above example, `BaseType` is always guaranteed to be erased and `someFunc` will be preserved under [`preserveValueImports`](../tsconfig/preserveValueImports.md), leaving us with the following code:
