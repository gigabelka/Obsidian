...
In TypeScript 3.1, mapped object types<sup>[1]</sup> over tuples and arrays now produce new tuples/arrays, rather than creating a new type where members like `push()`, `pop()`, and `length` are converted.
For example:
...
You can read [about it in the Publishing section of the declaration files section](../declaration-files/publishing.md#Version selection with `typesVersions`)
