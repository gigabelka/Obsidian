...
[`strictNullChecks`](../tsconfig/strictNullChecks.md) switches to a new strict null checking mode.
...
Type guards for dotted names also work with user defined type guard functions and the `typeof` and `instanceof` operators and do not depend on the [`strictNullChecks`](../tsconfig/strictNullChecks.md) compiler option.
...
Control flow based type analysis is particularly relevant in [`strictNullChecks`](../tsconfig/strictNullChecks.md) mode because nullable types are represented using union types:
...
Furthermore, in [`strictNullChecks`](../tsconfig/strictNullChecks.md) mode, control flow based type analysis includes _definite assignment analysis_ for local variables of types that don't permit the value `undefined`.
...
Glob-like file patterns are supported by two properties [`include`](../tsconfig/include.md) and [`exclude`](../tsconfig/exclude.md).
...
If a segment of a glob pattern includes only `*` or `.*`, then only files with supported extensions are included (e.g. `.ts`, `.tsx`, and `.d.ts` by default with `.js` and `.jsx` if [`allowJs`](../tsconfig/allowJs.md) is set to true).

If the [`files`](../tsconfig/files.md) and [`include`](../tsconfig/include.md) are both left unspecified, the compiler defaults to including all TypeScript (`.ts`, `.d.ts` and `.tsx`) files in the containing directory and subdirectories except those excluded using the [`exclude`](../tsconfig/exclude.md) property. JS files (`.js` and `.jsx`) are also included if [`allowJs`](../tsconfig/allowJs.md) is set to true.

If the [`files`](../tsconfig/files.md) or [`include`](../tsconfig/include.md) properties are specified, the compiler will instead include the union of the files included by those two properties.
Files in the directory specified using the [`outDir`](../tsconfig/outDir.md) compiler option are always excluded unless explicitly included via the [`files`](../tsconfig/files.md) property (even when the [`exclude`](../tsconfig/exclude.md) property is specified).

Files included using [`include`](../tsconfig/include.md) can be filtered using the [`exclude`](../tsconfig/exclude.md) property.
However, files included explicitly using the [`files`](../tsconfig/files.md) property are always included regardless of [`exclude`](../tsconfig/exclude.md).
The [`exclude`](../tsconfig/exclude.md) property defaults to excluding the `node_modules`, `bower_components`, and `jspm_packages` directories when not specified.
...
Using a [`baseUrl`](../tsconfig/baseUrl.md) is a common practice in applications using AMD module loaders where modules are "deployed" to a single folder at run-time.
All module imports with bare specifier names are assumed to be relative to the [`baseUrl`](../tsconfig/baseUrl.md).
...
The TypeScript compiler supports the declaration of such mappings using [`paths`](../tsconfig/paths.md) property in `tsconfig.json` files.
...
Using [`paths`](../tsconfig/paths.md) also allows for more sophisticated mappings including multiple fall back locations.
...
[`rootDirs`](../tsconfig/rootDirs.md) specify a list of _roots_ whose contents are expected to merge at run-time.
...
[`traceResolution`](../tsconfig/traceResolution.md) offers a handy way to understand how modules have been resolved by the compiler.
...
When compiled in [`strictNullChecks`](../tsconfig/strictNullChecks.md) mode, optional properties and methods automatically have `undefined` included in their type. Thus, the `b` property above is of type `number | undefined` and the `g` method above is of type `(() => number) | undefined`.
...
Enter [`lib`](../tsconfig/lib.md); with [`lib`](../tsconfig/lib.md) you can specify a list of built-in API declaration groups that you can choose to include in your project.
...
[`noUnusedParameters`](../tsconfig/noUnusedParameters.md) flags any unused function or method parameters errors.
[`noUnusedLocals`](../tsconfig/noUnusedLocals.md) flags any unused local (un-exported) declaration like variables, functions, classes, imports, etc...
Also, unused private members of a class would be flagged as errors under [`noUnusedLocals`](../tsconfig/noUnusedLocals.md).
...
TypeScript 2.0 adds a new [`skipLibCheck`](../tsconfig/skipLibCheck.md) compiler option that causes type checking of declaration files (files with extension `.d.ts`) to be skipped.
...
Since declarations in one file can affect type checking in other files, some errors may not be detected when [`skipLibCheck`](../tsconfig/skipLibCheck.md) is specified.
...
[`declarationDir`](../tsconfig/declarationDir.md) allows for generating declaration files in a different location than JavaScript files.
