...
TypeScript 2.7 introduces a new flag called [`strictPropertyInitialization`](../tsconfig/strictPropertyInitialization.md).
...
Keep in mind that [`strictPropertyInitialization`](../tsconfig/strictPropertyInitialization.md) will be turned on along with other [`strict`](../tsconfig/strict.md) mode flags, which can impact your project.
You can set the [`strictPropertyInitialization`](../tsconfig/strictPropertyInitialization.md) setting to `false` in your `tsconfig.json`'s `compilerOptions`, or `--strictPropertyInitialization false` on the command line to turn off this checking.
...
TypeScript 2.7 updates CommonJS/AMD/UMD module emit to synthesize namespace records based on the presence of an `__esModule` indicator under [`esModuleInterop`](../tsconfig/esModuleInterop.md).
...
Under the new [`esModuleInterop`](../tsconfig/esModuleInterop.md) these two issues should be addressed:
...
With [`esModuleInterop`](../tsconfig/esModuleInterop.md) two new helpers are generated `__importStar` and `__importDefault` for import `*` and import `default` respectively.
...
TypeScript's [`pretty`](../tsconfig/pretty.md) flag can make error messages easier to read and manage.
[`pretty`](../tsconfig/pretty.md) now uses colors for file names, diagnostic codes, and line numbers.
