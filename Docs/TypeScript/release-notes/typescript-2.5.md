...
TypeScript 2.5 brings the [`preserveSymlinks`](../tsconfig/preserveSymlinks.md) flag, which parallels the behavior of [the `--preserve-symlinks` flag in Node.js](https://nodejs.org/api/cli.html#cli_preserve_symlinks).
This flag also exhibits the opposite behavior to Webpack's `resolve.symlinks` option (i.e. setting TypeScript's [`preserveSymlinks`](../tsconfig/preserveSymlinks.md) to `true` parallels setting Webpack's `resolve.symlinks` to `false`, and vice-versa).
