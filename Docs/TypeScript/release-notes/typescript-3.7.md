...
That might result in dividing `undefined`, which is why in [`strictNullChecks`](../tsconfig/strictNullChecks.md), the following is an error.
...
The [`declaration`](../tsconfig/declaration.md) flag in TypeScript allows us to generate `.d.ts` files (declaration files) from TypeScript source files (i.e. `.ts` and `.tsx` files).
...
Unfortunately, [`declaration`](../tsconfig/declaration.md) didn't work with the [`allowJs`](../tsconfig/allowJs.md) flag which allows mixing TypeScript and JavaScript input files.
This was a frustrating limitation because it meant users couldn't use the [`declaration`](../tsconfig/declaration.md) flag when migrating codebases, even if they were JSDoc-annotated.
...
The way that this works is that when using [`allowJs`](../tsconfig/allowJs.md), TypeScript has some best-effort analyses to understand common JavaScript patterns; however, the way that some patterns are expressed in JavaScript don't necessarily look like their equivalents in TypeScript.
When [`declaration`](../tsconfig/declaration.md) emit is turned on, TypeScript figures out the best way to transform JSDoc comments and CommonJS exports into valid type declarations and the like in the output `.d.ts` files.
...
If you simply want TypeScript to create `.d.ts` files, you can use the [`emitDeclarationOnly`](../tsconfig/emitDeclarationOnly.md) compiler option.
...
We've provided a new flag called [`useDefineForClassFields`](../tsconfig/useDefineForClassFields.md) to enable this emit mode with some new checking logic.
...
Currently [`useDefineForClassFields`](../tsconfig/useDefineForClassFields.md) is only available when targeting ES5 and upwards, since `Object.defineProperty` doesn't exist in ES3.
To achieve similar checking for issues, you can create a separate project that targets ES5 and uses [`noEmit`](../tsconfig/noEmit.md) to avoid a full build.
...
We strongly encourage users to try the [`useDefineForClassFields`](../tsconfig/useDefineForClassFields.md) flag and report back on our issue tracker or in the comments below.
...
You can disable this behavior with the compiler option [`disableSourceOfProjectReferenceRedirect`](../tsconfig/disableSourceOfProjectReferenceRedirect.md) which may be appropriate when working in very large projects where this change may impact editing performance.
...
This error is only issued in `if` conditions, and it is not issued on optional properties, if [`strictNullChecks`](../tsconfig/strictNullChecks.md) is off, or if the function is later called within the body of the `if`:
...
Historically this comment was only respected in JavaScript source files in the presence of [`checkJs`](../tsconfig/checkJs.md), but we've expanded support to TypeScript files to make migrations easier for all users.
...
While not a breakage per se, opting in to the [`useDefineForClassFields`](../tsconfig/useDefineForClassFields.md) flag can cause breakage when:
...
- [`strictNullChecks`](../tsconfig/strictNullChecks.md) is disabled
