...
Use [`allowUnreachableCode`](../tsconfig/allowUnreachableCode.md) to disable unreachable code detection and reporting.
...
use [`allowUnusedLabels`](../tsconfig/allowUnusedLabels.md) to stop reporting these errors.
...
The check is turned _off_ by default; use [`noImplicitReturns`](../tsconfig/noImplicitReturns.md) to turn it on.
...
This check is turned _off_ by default, and can be enabled using [`noFallthroughCasesInSwitch`](../tsconfig/noFallthroughCasesInSwitch.md).

##### Example

With [`noFallthroughCasesInSwitch`](../tsconfig/noFallthroughCasesInSwitch.md), this example will trigger an error:
...
Specifying [`outFile`](../tsconfig/outFile.md) in conjunction with `--module amd` or `--module system` will concatenate all modules in the compilation into a single output file containing multiple module closures.

A module name will be computed for each module based on its relative location to [`rootDir`](../tsconfig/rootDir.md).
...
Setting the new compiler flag [`allowSyntheticDefaultImports`](../tsconfig/allowSyntheticDefaultImports.md) indicates that the module loader performs some kind of synthetic default import member creation not indicated in the imported .ts or .d.ts. The compiler will infer the existence of a `default` export that has the shape of the entire module itself.
...
The TypeScript compiler checks the input `.js` files for syntax errors, and emits valid output based on the [`target`](../tsconfig/target.md) and [`module`](../tsconfig/module.md) flags.
...
By just passing the [`pretty`](../tsconfig/pretty.md) command line option, TypeScript gives a more colorful output with context about where things are going wrong.
...
TypeScript 1.8 allows users to use the [`outFile`](../tsconfig/outFile.md) argument with special file system entities like named pipes, devices, etc.
