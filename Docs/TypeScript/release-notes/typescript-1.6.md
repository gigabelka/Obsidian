...
User however can override module resolution rules picked by the compiler by using [`moduleResolution`](../tsconfig/moduleResolution.md) command line option. Possible values are:
...
The [`exclude`](../tsconfig/exclude.md) list does not support wildcards. It must simply be a list of files and/or directories.
