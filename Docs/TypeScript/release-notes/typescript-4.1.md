...
For many users, this behavior was undesirable, and felt like it wasn't leveraging the full strict-checking of [`strictNullChecks`](../tsconfig/strictNullChecks.md).

That's why TypeScript 4.1 ships with a new flag called [`noUncheckedIndexedAccess`](../tsconfig/noUncheckedIndexedAccess.md).
...
One consequence of using [`noUncheckedIndexedAccess`](../tsconfig/noUncheckedIndexedAccess.md) is that indexing into an array is also more strictly checked, even in a bounds-checked loop.
...
This flag can be handy for catching out-of-bounds errors, but it might be noisy for a lot of code, so it is not automatically enabled by the [`strict`](../tsconfig/strict.md) flag; however, if this feature is interesting to you, you should feel free to try it and determine whether it makes sense for your team's codebase!
...
Unfortunately, specifying [`paths`](../tsconfig/paths.md) to enable path-mapping required also specifying an option called [`baseUrl`](../tsconfig/baseUrl.md), which allows bare specifier paths to be reached relative to the [`baseUrl`](../tsconfig/baseUrl.md) too.
This also often caused poor paths to be used by auto-imports.

In TypeScript 4.1, the [`paths`](../tsconfig/paths.md) option can be used without [`baseUrl`](../tsconfig/baseUrl.md).
This helps avoid some of these issues.

## `checkJs` Implies `allowJs`

Previously if you were starting a checked JavaScript project, you had to set both [`allowJs`](../tsconfig/allowJs.md) and [`checkJs`](../tsconfig/checkJs.md).
This was a slightly annoying bit of friction in the experience, so [`checkJs`](../tsconfig/checkJs.md) now implies [`allowJs`](../tsconfig/allowJs.md) by default.

[See more details at the pull request](https://github.com/microsoft/TypeScript/pull/40275).

## React 17 JSX Factories

TypeScript 4.1 supports React 17's upcoming `jsx` and `jsxs` factory functions through two new options for the [`jsx`](../tsconfig/jsx.md) compiler option:
