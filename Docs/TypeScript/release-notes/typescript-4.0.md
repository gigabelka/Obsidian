...
TypeScript 4.0 can now use control flow analysis to determine the types of properties in classes when [`noImplicitAny`](../tsconfig/noImplicitAny.md) is enabled.
...
In cases where you know better (e.g. you have an `initialize` method of some sort), you'll still need an explicit type annotation along with a definite assignment assertion (`!`) if you're in [`strictPropertyInitialization`](../tsconfig/strictPropertyInitialization.md).
...
While the types of `catch` variables won't change by default, we might consider a new [`strict`](../tsconfig/strict.md) mode flag in the future so that users can opt in to this behavior.
...
In TypeScript 4.0, users can customize the fragment factory through the new [`jsxFragmentFactory`](../tsconfig/jsxFragmentFactory.md) option.
...
Previously, compiling a program after a previous compile with errors under [`incremental`](../tsconfig/incremental.md) would be extremely slow when using the [`noEmitOnError`](../tsconfig/noEmitOnError.md) flag.
This is because none of the information from the last compilation would be cached in a `.tsbuildinfo` file based on the [`noEmitOnError`](../tsconfig/noEmitOnError.md) flag.

TypeScript 4.0 changes this which gives a great speed boost in these scenarios, and in turn improves `--build` mode scenarios (which imply both [`incremental`](../tsconfig/incremental.md) and [`noEmitOnError`](../tsconfig/noEmitOnError.md)).

For details, [read up more on the pull request](https://github.com/microsoft/TypeScript/pull/38853).

## `--incremental` with `--noEmit`

TypeScript 4.0 allows us to use the [`noEmit`](../tsconfig/noEmit.md) flag while still leveraging [`incremental`](../tsconfig/incremental.md) compiles.
This was previously not allowed, as [`incremental`](../tsconfig/incremental.md) needs to emit a `.tsbuildinfo` files; however, the use-case to enable faster incremental builds is important enough to enable for all users.
...
Previously, it was only an error for properties to override accessors, or accessors to override properties, when using [`useDefineForClassFields`](../tsconfig/useDefineForClassFields.md); however, TypeScript now always issues an error when declaring a property in a derived class that would override a getter or setter in the base class.
...
When using the `delete` operator in [`strictNullChecks`](../tsconfig/strictNullChecks.md), the operand must now be `any`, `unknown`, `never`, or be optional (in that it contains `undefined` in the type).
