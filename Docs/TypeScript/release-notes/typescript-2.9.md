...
- Otherwise use [`keyofStringsOnly`](../tsconfig/keyofStringsOnly.md) compiler option to disable the new behavior.
...
Often in Node.js applications a `.json` is needed. With TypeScript 2.9, [`resolveJsonModule`](../tsconfig/resolveJsonModule.md) allows for importing, extracting types from and generating `.json` files.
...
Starting TypeScript 2.9 errors are displayed under [`pretty`](../tsconfig/pretty.md) by default if the output device is applicable for colorful text.
TypeScript will check if the output stream has [`isTty`](https://nodejs.org/api/tty.html) property set.

Use `--pretty false` on the command line or set `"pretty": false` in your `tsconfig.json` to disable [`pretty`](../tsconfig/pretty.md) output.

## New `--declarationMap`

Enabling [`declarationMap`](../tsconfig/declarationMap.md) alongside [`declaration`](../tsconfig/declaration.md) causes the compiler to emit `.d.ts.map` files alongside the output `.d.ts` files.
...
In other words, hitting go-to-definition on a declaration from a `.d.ts` file generated with [`declarationMap`](../tsconfig/declarationMap.md) will take you to the source file (`.ts`) location where that declaration was defined, and not to the `.d.ts`.
