...
> We also need to make sure that TypeScript knows `Promise` exists by setting your [`lib`](../tsconfig/lib.md) option to something like `"dom", "es2015"` or `"dom", "es2015.promise", "es5"`
...
2.  no helpers at all with [`noEmitHelpers`](../tsconfig/noEmitHelpers.md).
...
Second, compile your files using [`importHelpers`](../tsconfig/importHelpers.md):
...
An import to a module with no declaration file will still be flagged as an error under [`noImplicitAny`](../tsconfig/noImplicitAny.md).
...
This is only enabled if [`noImplicitAny`](../tsconfig/noImplicitAny.md) is set.
...
One great benefit of this is that you'll see _way fewer_ implicit `any` errors when running with [`noImplicitAny`](../tsconfig/noImplicitAny.md).
...
- `extends` is a new top-level property in `tsconfig.json` (alongside `compilerOptions`, [`files`](../tsconfig/files.md), [`include`](../tsconfig/include.md), and [`exclude`](../tsconfig/exclude.md)).
...
- [`files`](../tsconfig/files.md), [`include`](../tsconfig/include.md), and [`exclude`](../tsconfig/exclude.md) from the inheriting config file _overwrite_ those from the base config file.
...
Invoking the compiler with [`alwaysStrict`](../tsconfig/alwaysStrict.md) causes:
