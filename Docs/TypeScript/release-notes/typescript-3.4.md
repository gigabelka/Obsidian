...
TypeScript 3.4 introduces a new flag called [`incremental`](../tsconfig/incremental.md) which tells TypeScript to save information about the project graph from the last compilation.
The next time TypeScript is invoked with [`incremental`](../tsconfig/incremental.md), it will use that information to detect the least costly way to type-check and emit changes to your project.
...
We can also name them anything that we want, and place them anywhere we want using the [`tsBuildInfoFile`](../tsconfig/tsBuildInfoFile.md) option.
...
Part of the intent with composite projects (`tsconfig.json`s with [`composite`](../tsconfig/composite.md) set to `true`) is that references between different projects can be built incrementally.
As such, composite projects will **always** produce `.tsbuildinfo` files.

### `outFile`

When [`outFile`](../tsconfig/outFile.md) is used, the build information file's name will be based on the output file's name.
As an example, if our output JavaScript file is `./output/foo.js`, then under the [`incremental`](../tsconfig/incremental.md) flag, TypeScript will generate the file `./output/foo.tsbuildinfo`.
As above, this can be controlled with the [`tsBuildInfoFile`](../tsconfig/tsBuildInfoFile.md) option.
