...
TypeScript 3.2 introduces a new [`strictBindCallApply`](../tsconfig/strictBindCallApply.md) compiler option (in the [`strict`](../tsconfig/strict.md) family of options) with which the `bind`, `call`, and `apply` methods on function objects are strongly typed and strictly checked.
...
For those purposes you may want to add `esnext.bigint` to the [`lib`](../tsconfig/lib.md) setting in your compiler options.
...
When writing in JavaScript files (using [`allowJs`](../tsconfig/allowJs.md)), TypeScript now recognizes declarations that use `Object.defineProperty`.
This means you'll get better completions, and stronger type-checking when enabling type-checking in JavaScript files (by turning on the [`checkJs`](../tsconfig/checkJs.md) option or adding a `// @ts-check` comment to the top of your file).
