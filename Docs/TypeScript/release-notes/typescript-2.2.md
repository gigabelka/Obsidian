...
Note that the union type case only occurs in [`strictNullChecks`](../tsconfig/strictNullChecks.md) mode because `null` and `undefined` disappear from unions in classic type checking mode.
...
The new [`jsx`](../tsconfig/jsx.md) value `react-native` will preserve the JSX syntax in the output file, but give it a `.js` extension.
