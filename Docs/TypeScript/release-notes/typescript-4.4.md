...
That's why TypeScript 4.4 introduces a new flag called [`useUnknownInCatchVariables`](../tsconfig/useUnknownInCatchVariables.md).
...
This flag is enabled under the [`strict`](../tsconfig/strict.md) family of options.
That means that if you check your code using [`strict`](../tsconfig/strict.md), this option will automatically be turned on.
...
In TypeScript 4.4, the new flag [`exactOptionalPropertyTypes`](../tsconfig/exactOptionalPropertyTypes.md) specifies that optional property types should be interpreted exactly as written, meaning that `| undefined` is not added to the type:
...
This flag is **not** part of the [`strict`](../tsconfig/strict.md) family and needs to be turned on explicitly if you'd like this behavior.
It also requires [`strictNullChecks`](../tsconfig/strictNullChecks.md) to be enabled as well.
...
These changes can improve TypeScript's general performance in code with fairly complex types, and is especially observed when emitting `.d.ts` files under the [`declaration`](../tsconfig/declaration.md) flag.
...
TypeScript now caches the way it constructs path-mappings (using the [`paths`](../tsconfig/paths.md) option in `tsconfig.json`).
...
In what was effectively a bug, TypeScript would end up redoing type-checking work under [`incremental`](../tsconfig/incremental.md) compilations if [`strict`](../tsconfig/strict.md) was on.
This led to many builds being just as slow as if [`incremental`](../tsconfig/incremental.md) was turned off.
...
When performing a [`--force`](../tsconfig/force.md) build, however, that information is irrelevant since every project dependency will be rebuilt from scratch.
In TypeScript 4.4, [`--force`](../tsconfig/force.md) builds avoid those unnecessary steps and start a full build.
...
That's why TypeScript now issues spelling suggestions in plain JavaScript files - ones without `// @ts-check` or in a project with [`checkJs`](../tsconfig/checkJs.md) turned off.
...
Users running with the [`strict`](../tsconfig/strict.md) flag may see new errors around `catch` variables being `unknown`, especially if the existing code assumes only `Error` values have been caught.
...
Otherwise, you can just use a type assertion, add an explicit `: any` to your catch variable, or turn off [`useUnknownInCatchVariables`](../tsconfig/useUnknownInCatchVariables.md).
