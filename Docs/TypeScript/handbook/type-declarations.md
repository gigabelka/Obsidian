...
This is one function of the [`target`](../tsconfig/target.md) compiler setting.

TypeScript helps with this problem by varying which `lib` files are included by default based on your [`target`](../tsconfig/target.md) setting.
For example, if [`target`](../tsconfig/target.md) is `ES5`, you will see an error if trying to use the `startsWith` method, because that method is only available in `ES6` or later.

### `lib` setting

The [`lib`](../tsconfig/lib.md) setting allows more fine-grained control of which built-in declaration files are considered available in your program.
See the documentation page on [`lib`](../tsconfig/lib.md) for more information.
...
If you're a package author considering bundling type definitions with your package, you can read our guide on [bundling type definitions](../declaration-files/publishing.md#Including declarations in your npm package).
...
See the appendix [Writing Declaration Files](../declaration-files/introduction.md) for a guide.
