...
We can also read from those properties - but when we do under [`strictNullChecks`](../tsconfig/strictNullChecks.md), TypeScript will tell us they're potentially `undefined`.
...
Using [mapping modifiers](type-manipulation/mapped-types.md#Mapping Modifiers), you can remove `readonly` attributes.
