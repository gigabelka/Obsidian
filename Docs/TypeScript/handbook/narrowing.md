...
In addition, classes can [use `this is Type`](classes.md#`this`-based type guards) to narrow their type.

## Assertion functions

Types can also be narrowed using [Assertion functions](../release-notes/typescript-3.7.md#Assertion Functions).
...
Under [`strictNullChecks`](../tsconfig/strictNullChecks.md) that gives us an error - which is appropriate since `radius` might not be defined.
...
Additionally, outside of [`strictNullChecks`](../tsconfig/strictNullChecks.md) we're able to accidentally access any of those fields anyway (since optional properties are just assumed to always be present when reading them).
...
When `radius` was optional, we got an error (with [`strictNullChecks`](../tsconfig/strictNullChecks.md) enabled) because TypeScript couldn't tell whether the property was present.
...
Both interpretations are correct, but only the union encoding of `Shape` will cause an error regardless of how [`strictNullChecks`](../tsconfig/strictNullChecks.md) is configured.
