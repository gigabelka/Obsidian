...
The [`strictPropertyInitialization`](../tsconfig/strictPropertyInitialization.md) setting controls whether class fields need to be initialized in the constructor.
...
Classes can declare index signatures; these work the same as [Index Signatures for other object types](object-types.md#Index Signatures):
...
JavaScript classes are instantiated with the `new` operator. Given the type of a class itself, the [InstanceType](../reference/utility-types.md#`InstanceType<Type>`) utility type models this operation.
