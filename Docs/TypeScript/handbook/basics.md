...
In that case, you can use the [`noEmitOnError`](../tsconfig/noEmitOnError.md) compiler option.
...
We could have chosen something a little bit more recent by using the [`target`](../tsconfig/target.md) option.
...
The [`strict`](../tsconfig/strict.md) flag in the CLI, or `"strict": true` in a [`tsconfig.json`](../project-config/tsconfig.json.md) toggles them all on simultaneously, but we can opt out of them individually.
The two biggest ones you should know about are [`noImplicitAny`](../tsconfig/noImplicitAny.md) and [`strictNullChecks`](../tsconfig/strictNullChecks.md).
...
Turning on the [`noImplicitAny`](../tsconfig/noImplicitAny.md) flag will issue an error on any variables whose type is implicitly inferred as `any`.
...
The [`strictNullChecks`](../tsconfig/strictNullChecks.md) flag makes handling `null` and `undefined` more explicit, and _spares_ us from worrying about whether we _forgot_ to handle `null` and `undefined`.
