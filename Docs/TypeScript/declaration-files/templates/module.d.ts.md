...
See [Module: Functions](module-function.d.ts.md) for details of how that works, and the [Modules reference](../../modules-reference/introduction.md) page.
...
This example is a good case for [using generics](../../handbook/type-manipulation/generics.md#Generic Types) to provide richer type information:
...
The types which are exported can then be re-used by consumers of the modules using either `import` or `import type` in TypeScript code or [JSDoc imports](../../javascript/jsdoc-reference.md#Import types).
...
To understand how namespaces work in `.d.ts` files read the [`.d.ts` deep dive](../deep-dive.md).
