...
If your types are generated by your source code, publish the types with your source code. Both TypeScript and JavaScript projects can generate types via [`declaration`](../tsconfig/declaration.md).
...
That's what that `{ "*": ["ts3.1/*"] }` means - if you're familiar with [path mapping](../tsconfig/paths.md), it works exactly like that.
