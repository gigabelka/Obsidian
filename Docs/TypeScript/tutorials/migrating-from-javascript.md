...
1. Read in any files it understands in the `src` directory (with [`include`](../tsconfig/include.md)).
2. Accept JavaScript files as inputs (with [`allowJs`](../tsconfig/allowJs.md)).
3. Emit all of the output files in `built` (with [`outDir`](../tsconfig/outDir.md)).
4. Translate newer JavaScript constructs down to an older version like ECMAScript 5 (using [`target`](../tsconfig/target.md)).
...
- [`noImplicitReturns`](../tsconfig/noImplicitReturns.md) which prevents you from forgetting to return at the end of a function.
- [`noFallthroughCasesInSwitch`](../tsconfig/noFallthroughCasesInSwitch.md) which is helpful if you never want to forget a `break` statement between `case`s in a `switch` block.

TypeScript will also warn about unreachable code and labels, which you can disable with [`allowUnreachableCode`](../tsconfig/allowUnreachableCode.md) and [`allowUnusedLabels`](../tsconfig/allowUnusedLabels.md) respectively.
...
If, for instance, you _don't_ want TypeScript to compile to JavaScript in the face of errors, you can use the [`noEmitOnError`](../tsconfig/noEmitOnError.md) option.
...
For instance, if you never want TypeScript to silently infer `any` for a type without you explicitly saying so, you can use [`noImplicitAny`](../tsconfig/noImplicitAny.md) before you start modifying your files.
...
First, you'll need to enable some module system by setting TypeScript's [`module`](../tsconfig/module.md) option.
...
If you're using a module option other than `commonjs`, you'll need to set your [`moduleResolution`](../tsconfig/moduleResolution.md) option to `node`.
...
You can tell TypeScript to flag these locations down and give an error with the [`noImplicitAny`](../tsconfig/noImplicitAny.md) option.
...
Since `null` and `undefined` are such a frequent source of bugs in JavaScript and TypeScript, TypeScript has the [`strictNullChecks`](../tsconfig/strictNullChecks.md) option to spare you the stress of worrying about these issues.

When [`strictNullChecks`](../tsconfig/strictNullChecks.md) is enabled, `null` and `undefined` get their own types called `null` and `undefined` respectively.
...
As a heads up, when using [`strictNullChecks`](../tsconfig/strictNullChecks.md), your dependencies may need to be updated to use [`strictNullChecks`](../tsconfig/strictNullChecks.md) as well.
...
For this reason, TypeScript has the [`noImplicitThis`](../tsconfig/noImplicitThis.md) option.
