...
- [`noEmitOnError`](../tsconfig/noEmitOnError.md) : Do not emit outputs if any errors were reported.
- [`noImplicitAny`](../tsconfig/noImplicitAny.md) : Raise error on expressions and declarations with an implied `any` type.
- [`sourceMap`](../tsconfig/sourceMap.md) : Generates corresponding `.map` file.
- [`target`](../tsconfig/target.md) : Specify ECMAScript target version.

Note: `"ESNext"` targets latest supported

[`noImplicitAny`](../tsconfig/noImplicitAny.md) is good idea whenever you’re writing new code — you can make sure that you don’t write any untyped code by mistake. `"compileOnSave"` makes it easy to update your code in a running web app.
