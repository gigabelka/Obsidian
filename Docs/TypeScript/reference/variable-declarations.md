...
[As we mentioned earlier](../handbook/everyday-types.md), `let` is similar to `var` in some respects, but allows users to avoid some of the common "gotchas" that users run into in JavaScript.
...
The [chapter on Interfaces](../handbook/object-types.md) has the details.
