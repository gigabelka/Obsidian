...
As of TypeScript 5.5, the compiler does not generate reference directives, and does _not_ emit handwritten triple-slash directives to output files unless those directives are marked as [`preserve="true"`](#`preserve="true"`).
...
They also serve as a method to order the output when using [`out`](../tsconfig/out.md) or [`outFile`](../tsconfig/outFile.md).
...
these are the file names specified on the command-line or in the [`files`](../tsconfig/files.md) list in the `tsconfig.json` file.
...
If the compiler flag [`noResolve`](../tsconfig/noResolve.md) is specified, triple-slash references are ignored; they neither result in adding new files, nor change the order of the files provided.
...
For declaring a dependency on an `@types` package in a `.ts` file, use [`types`](../tsconfig/types.md) on the command line or in your `tsconfig.json` instead.
See [using `@types`, `typeRoots` and `types` in `tsconfig.json` files](../project-config/tsconfig.json.md) for more details.

## `/// <reference lib="..." />`

This directive allows a file to explicitly include an existing built-in _lib_ file.

Built-in _lib_ files are referenced in the same fashion as the [`lib`](../tsconfig/lib.md) compiler option in _tsconfig.json_ (e.g. use `lib="es2015"` and not `lib="lib.es2015.d.ts"`, etc.).
...
The impact here is similar to passing [`noLib`](../tsconfig/noLib.md) on the command line.

Also note that when passing [`skipDefaultLibCheck`](../tsconfig/skipDefaultLibCheck.md), the compiler will only skip checking files with `/// <reference no-default-lib="true"/>`.
